...
def ensure_upstream_license(
    ...
    with session.get(url, stream=True, timeout=(30, 300)) as response:
        ...
        with tempfile.TemporaryDirectory(prefix="devstral2-license-") as tmp:
...
def manifest_exists(tag: str, name: str) -> bool:
    with tempfile.TemporaryDirectory(prefix="devstral2-manifest-check-") as tmp:
        result = subprocess.run(
            [
                "gh",
                "release",
...
def stream_one(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="devstral2-mirror-") as tmp:
        tmpdir = Path(tmp)
