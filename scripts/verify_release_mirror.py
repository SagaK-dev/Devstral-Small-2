...
def main() -> None:
    ...
    metadata = json.loads(Path(args.upstream_metadata).read_text(encoding="utf-8"))
    ...
    manifests = sorted(Path(args.manifests_dir).glob("manifest-*.json"))
    ...
    if duplicates:
        raise SystemExit("Duplicate mirrored paths: " + ", ".join(sorted(set(duplicates))))

    missing_paths = sorted(path for path in expected if path not in entries)
    ...
    if "UPSTREAM_LICENSE_NOTICE.txt" not in assets:
        raise SystemExit("GitHub Release does not contain UPSTREAM_LICENSE_NOTICE.txt")
